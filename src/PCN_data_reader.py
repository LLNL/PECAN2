...
class Dataset_MLHDF(Dataset):
	def __init__(self, csv_path, mlhdf_path, cmlhdf_path, max_atoms=2500, feat_dim=22, feat_int=False, affine_trans=True):
		...
		if self.cmlhdf_path is not None and len(self.cmlhdf_path) > 1:
	...
	def __getitem__(self, idx):
		...
		#print(pdbid, poseid, affinity, poselabel)
		if poseid == 0:
			mlhdf_ds = self.cmlhdf[pdbid]["pybel"]["processed"]["crystal"]
		else:
			# if pdbid == "0043" and poseid == "10":
			# 	print("here!!!")
			# 	print(self.mlhdf[pdbid]["pybel"]["processed"]["docking"].keys())
			# 	exit()
