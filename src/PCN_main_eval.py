...
parser.add_argument("--max-atoms", type=int, default=2500, help="maximum number of atoms")
parser.add_argument("--batch-size", type=int, default=40, help="mini-batch size")
...
def eval():
	...
	dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=num_workers, worker_init_fn=None)
	...
	np.save(args.result_dir + "/" + args.model_name + "_eval_label.npy", ytrue_arr)
	np.save(args.result_dir + "/" + args.model_name + "_eval_pred.npy", ypred_arr)
	np.save(args.result_dir + "/" + args.model_name + "_eval_pred2.npy", ypred_arr2)
